+++
title = 'MIT 6.S184 第 1 章学习笔记｜生成建模即采样'
date = 2026-09-01
draft = false
summary = '生成模型的核心任务不是寻找唯一的「最佳答案」，而是学习一个未知的数据分布，并从中产生新的样本。本文详细介绍生成式 AI 的基本概念、数学表述和常见误区。'
tags = ['生成模型', '概率采样', 'Flow Matching', 'Diffusion Models', 'MIT-6.S184', '深度学习']
...
本文是 **MIT 6.S184 Flow Matching 与 Diffusion Models 学习笔记系列**的第 1 章。

- **第 1 章：生成建模即采样**（本文）
- 第 2 章：流模型与扩散模型（即将发布）
- 第 3 章：Flow Matching 详解（即将发布）
...
1. 纯坐标表示省略了原子种类、键连接等信息，真实的分子生成模型通常还要额外建模这些离散属性
2. 分子坐标带有平移、旋转等对称性，实际系统会用等变网络处理，这不影响本章「对象即向量」的抽象

**文本是重要例外：** token 属于有限词表，通常是离散对象。语言模型内部的 embedding 虽然是连续向量，但最终生成的 token 状态空间仍然是离散的，因此不能直接套用连续扩散的做法。
...
对连续数据，$p_{\mathrm{data}}(z)$ 是一个**概率密度**而不是概率：单个点的概率为 0，密度只在积分之后才对应概率。密度值大表示 $z$ 在所建模的分布下更「典型」。
...
| 评价方式 | 准确率、误差等逐样本指标 | FID、CLIP score 等分布级指标 |
...
<details>
<summary><strong>Q1：为什么生成一张狗的图片不是一个普通的「预测唯一答案」问题？</strong></summary>

**答案：** 因为符合要求的狗图有无穷多张，不存在唯一正确输出。用一个确定的函数去逼近会得到「所有狗图的平均」这种无意义结果；正确做法是用图像空间上的概率分布来描述这种多样性，并从中采样。
...
<details>
<summary><strong>Q2：数据集和 p_data 有什么区别？为什么这个区别重要？</strong></summary>
...
<details>
<summary><strong>Q3：p_data(z) 是概率还是概率密度？为什么要区分？</strong></summary>
...
<details>
<summary><strong>Q4：图像、视频、分子分别是几维向量？统一表示是什么？</strong></summary>
...
<details>
<summary><strong>Q5：条件生成需要模型学习什么？和无条件生成的关系是什么？</strong></summary>
...
<details>
<summary><strong>Q6：为什么连续扩散方法不能直接把 token ID 当实数加高斯噪声？</strong></summary>
...
<details>
<summary><strong>Q7：token embedding 是连续向量，为什么仍说文本是离散数据？</strong></summary>
...
<details>
<summary><strong>Q8：本章有没有给出任何生成算法？如果没有，缺的是什么？</strong></summary>
...
**误解 5：概率密度高就等于样本质量好**
- **纠正：** 密度只反映在所建模分布下的典型程度。高密度区域可能对应模糊的「平均样本」，而人类偏好的样本未必位于密度最高处——这正是第 5 章 Guidance 要处理的问题
...
- [huggingface/diffusers](https://github.com/huggingface/diffusers)：主流扩散/流模型的统一实现，便于把本章的抽象定义对上真实 API
- [facebookresearch/flow_matching](https://github.com/facebookresearch/flow_matching)：Flow Matching 官方实现，可作为第 3 章之后的落地参考
...
本文学习笔记基于 MIT 6.S184 课程讲义：

- **课程主页**：[MIT 6.S184: Generative AI](https://diffusion.csail.mit.edu/)
- **讲义来源**：*An Introduction to Flow Matching and Diffusion Models*（课程官方讲义第 1 章，PDF 第 3-6 页）
- **课程讲师**：MIT CSAIL
- **笔记版本**：v1.1（完成时间：2026-08-27，重排与勘误：2026-09-01）
